...
export default function WritersDashboard({ userId, navigate }) {
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen text-neutral-900 wd">
        <div className="wd-container">
        <Toaster richColors position="top-center" />
        <Nav active="writers" navigate={navigate} />
          ...
          <Card className="lg:col-span-1 shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <div className="text-6xl font-mono tabular-nums tracking-tight">{minutes}:{seconds}</div>
            ...
            <CardContent className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={wordsLast14}>
                  ...
                  />
                  <Bar dataKey="words" fill="#4f46e5" radius={[6,6,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <div className="wd-grid-3">
          {/* Projects */}
          <Card className="wd-col-span-2-md shadow-sm hover:shadow-md transition-shadow">
...
function PromptBox(){
  ...
  async function roll(){
    ...
    try {
      ...
      // Array-of-parts content (some models return structured content)
      if (!content && Array.isArray(msgContent)) {
        content = msgContent
          .map(part => {
            if (typeof part === "string") return part;
            if (typeof part?.text === "string") return part.text;
            if (typeof part?.content === "string") return part.content;
            if (Array.isArray(part?.content)) return part.content.map(pp => pp?.text || "").join("");
            return "";
          })
          .join("")
          .trim();
      ...
      if (!content && typeof choice?.content === "string") {
        content = choice.content.trim();
      }
      if (!content && Array.isArray(choice?.content)) {
      ...
      if (!content) {
        if (import.meta.env.DEV) console.warn("OpenAI returned no content", data);
